...
func main() {
	...
	// 3. 运行任务（自动崩溃恢复 + 断点续传）
	if err := task.Run(ctx, repo, eng, "export-001"); err != nil {
		panic(err)
	}
...
```go
task.Run(ctx, repo, eng, "task-001",
task.WithRetry(3, 5*time.Second), // 最多重试 3 次，间隔 5 秒
)
```
